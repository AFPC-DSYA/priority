...
const webpackConfig = merge(baseWebpackConfig, {
  ...
  devtool: config.buildLocal.productionSourceMap ? config.buildLocal.devtool : false,
  output: {
    path: config.buildLocal.assetsRoot,
  ...
  plugins: [
    ...
    new OptimizeCSSPlugin({
      cssProcessorOptions: config.buildLocal.productionSourceMap
    ...
    new CopyWebpackPlugin([
      {
        ...
        to: config.buildLocal.assetsSubDirectory,
        ignore: ['.*']
      }
    ])
  ]
...
if (config.buildLocal.productionGzip) {
  ...
  webpackConfig.plugins.push(
    new CompressionWebpackPlugin({
      ...
      test: new RegExp(
        '\\.(' +
        config.buildLocal.productionGzipExtensions.join('|') +
        ')$'
      ),
      threshold: 10240,
      minRatio: 0.8
...
if (config.buildLocal.bundleAnalyzerReport) {
